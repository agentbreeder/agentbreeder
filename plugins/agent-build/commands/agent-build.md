...
| CLOUD_PREFERENCE is GCP | **gemini-2.5-flash** (default — free-tier eligible) or **gemini-2.5-pro** (complex, requires billing) |
...
```yaml
prompts:
  system: prompts/<agent-name>-system     # resolved via engine.prompt_resolver
```

The actual prompt body lives in `prompts/<agent-name>-system.md` (see Step 2a-bis). The reference resolves via the chain: local file → `AGENTBREEDER_REGISTRY_URL`/api/v1/registry/prompts → inline-literal fallback. Never inline a multi-line prompt in `agent.yaml`.

Tools (`tools` list) must be **registry references**, not inline definitions:

```yaml
tools:
  - ref: tools/web-search           # engine.tools.standard.web_search (Tavily)
  - ref: tools/markdown-writer      # engine.tools.standard.markdown_writer
  # - ref: tools/<custom-kebab>     # local override at ./tools/<snake>.py
```

Resolution chain (see `engine.tool_resolver`): `./tools/<snake_name>.py` (local override) → `engine.tools.standard.<snake_name>` → registry API metadata. Always prefer standard-library tools — only generate a local file when no standard tool covers the use case.

### 2a-bis. `prompts/<agent-name>-system.md`

Always generate. This is the actual system prompt body, **tailored to the stated purpose** — never a placeholder. Use markdown for structure (lists, headings as needed). The prompt-registry registration script (`scripts/register_prompt.sh`) reads this file verbatim when pushing to the API.

The prompt body should reference tools by their **resolved snake_case names** (e.g., `web_search`, `markdown_writer`) — those are what the LLM sees in the function-calling schema, not the kebab-case YAML refs.

### 2b. `agent.py`
Working agent code for the chosen framework. **Resolve tools from the registry** rather than defining them inline. Include `if __name__ == "__main__":` block.
...
- **Google ADK:** `from google.adk.agents import Agent`; export module-level `root_agent = Agent(name="...", model="...", instruction=INSTRUCTION, tools=[...])`. Tools are plain Python functions with Google-style docstrings — NO `@adk.tool` decorator (it does not exist). The AgentBreeder runtime wrapper at `engine/runtimes/templates/google_adk_server.py` looks for the `root_agent` symbol — name it exactly that. **Resolve `INSTRUCTION` from the prompt registry** and **tools from the tool registry** at module load:
  ```python
  from engine.prompt_resolver import resolve_prompt
  from engine.tool_resolver import resolve_tool
  INSTRUCTION = resolve_prompt("prompts/<agent-name>-system", project_root=Path(__file__).resolve().parent)
  tool_a = resolve_tool("tools/<kebab-tool-name>", project_root=Path(__file__).resolve().parent)
  ```
  Include a fallback `try/except ImportError` block for `prompt_resolver` (file-only) so the agent still works when the engine package is not on `sys.path`. `tool_resolver` is a hard dep — surface a clear ImportError pointing at `pip install -e <agentbreeder-repo>`.
- **Custom:** minimal wrapper with HTTP entrypoint

**Tool selection — prefer registry tools over per-agent files.** Before generating any local tool stub, check the standard library: `engine/tools/standard/` has `web_search` (Tavily), `markdown_writer` (file persist), and others. Reference them from `agent.yaml` with `ref: tools/<kebab-name>` and from `agent.py` with `resolve_tool("tools/<kebab-name>")`. Only generate a local `tools/<snake_name>.py` file when the agent needs a domain-specific tool that no standard tool covers — and even then, name it generically so it can be promoted to `engine/tools/standard/` later. Never generate stubs that just `raise NotImplementedError` — implement the tool fully or skip it.

When a local tool is genuinely needed, scaffold it as:
...
# JSON-Schema (OpenAPI subset) for the tool registry — mirrors the function signature.
SCHEMA: dict[str, Any] = {
    "type": "object",
    "properties": {
...
def <snake_tool_name>(input: str) -> dict[str, Any]:
    ...
    """
    # Real implementation here. Do NOT raise NotImplementedError.
    ...
```
...
- Always: `agentbreeder-sdk>=1.5.0`, `pytest>=8.0.0`

### 2c-bis. `pyproject.toml`

Always generate. This puts the project root on `sys.path` so `import agent` works in tests, and stops pytest from inheriting config (e.g. `asyncio_mode`) from any parent repo if the project is scaffolded inside another git repo.
...
- Always: `AGENT_AUTH_TOKEN=` (bearer token gate for `/invoke` and `/stream` — leave empty for local dev to disable auth, set in production via secrets manager). Add `AGENT_AUTH_TOKEN` to the `deploy.secrets` list in `agent.yaml` so the deployer injects it from Secret Manager / AWS Secrets / Vault at runtime.
...
- Import: `from google.adk.agents import Agent` (NOT `import adk`)
- Export the module-level symbol `root_agent` — the AgentBreeder runtime wrapper looks for it by name
- Tools are plain Python functions with Google-style docstrings (Args/Returns) — there is NO `@adk.tool` decorator
- Session backend set in `agent.yaml` (google_adk.session_backend), not in code
- Serve via `engine/runtimes/templates/google_adk_server.py` (production) or `adk api_server` (dev). `python agent.py` is for smoke checks only
- Memory service configured in `agent.yaml` (google_adk.memory_service)
- Test with `InMemoryRunner` locally before containerizing
- Never hardcode project ID or region — use env vars
- For free-tier Google AI Studio API keys, use `gemini-2.5-flash` — `gemini-2.5-pro` and `gemini-2.0-flash` have a 0-quota free tier (billing required)
```
