...
- **Codex** reads `AGENTS.md` — **generated by 100xprism from `CLAUDE.md`; never hand-edit it.** Put rules in `CLAUDE.md` and regenerate `AGENTS.md` via `/update-claude` so both harnesses stay aligned.
...
- **Update the guides in BOTH repos**: any feature that changes cross-repo behaviour MUST update `CLAUDE.md` in **both** `agentbreeder` and `agentbreeder-cloud`, spelling out *what changes on each side* (see "Documenting a cross-repo feature" below). `AGENTS.md` is generated by 100xprism (`# Source of truth: modules/<slug>/SKILL.md`) — **do not hand-edit it**; put the rule in `CLAUDE.md` and regenerate `AGENTS.md` (`/update-claude`) so both AI harnesses stay in sync.
...
9. **Sync the guides both ways** — if the feature touches Cloud or the website, update `CLAUDE.md` in **both** repos per the Cross-Repo Sync table above (what changes OSS→Cloud, Cloud→OSS, and →Website), then regenerate `AGENTS.md` (`/update-claude`) rather than hand-editing it.
