...
- Local stack running: `docker compose -f deploy/docker-compose.yml up -d`
:::
...
<Tabs items={['Studio', 'API', 'agent.yaml']}>
  <Tab value="Studio">
...
Studio shows extraction progress:
```
...
```bash
docker compose -f deploy/docker-compose.yml --profile graphrag up
```
