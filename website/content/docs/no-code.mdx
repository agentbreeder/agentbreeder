...
> **Status:** Non-local targets in the table below work via existing infrastructure today. Greenfield provisioning (auto-creating VPCs/clusters/resource-groups) is fully supported only for `local` and `cloud-run`. `claude-managed` is an Anthropic-managed runtime. See the [deploy-target status table](cli-reference#agentbreeder-deploy).
...
This matrix applies to No Code, Low Code, and Full Code equally.
