...
> **Destination-platform availability:** Upgrading assumes your target deploy platform is shipped. Not every deploy target listed in `agent.yaml` carries the same governance today — see the [deploy-target status table](/docs/cli-reference#agentbreeder-deploy) for current per-target status, and the [prerequisites matrix](/docs/deployment#prerequisites-per-target-as-of-2026-05-18) for setup requirements before you upgrade.
