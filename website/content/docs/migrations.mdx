...
> **Destination-platform availability:** Migration guides assume your target deploy platform is GA. Not every deploy target listed in `agent.yaml` is fully shipped — see the [deploy-target status table](/docs/cli-reference#agentbreeder-deploy) for what's GA vs. beta vs. coming soon, and the [prerequisites matrix](/docs/deployment#prerequisites-per-target-as-of-2026-05-18) for setup requirements before you migrate.
