...
> **Status:** Shipped status varies by target. Today, Local and GCP Cloud Run (greenfield) are the smoothest paths; others require pre-existing infrastructure or are partial. See the [deploy-target status table](cli-reference#agentbreeder-deploy) and [per-target prerequisites](deployment#prerequisites-per-target-as-of-2026-05-18).
