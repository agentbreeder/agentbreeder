...
> **Status:** Deployer code paths exist for every value listed below. Greenfield provisioning is fully supported only for `local` and `cloud-run`; AWS, Azure, and Kubernetes targets require pre-existing infrastructure. `claude-managed` is an Anthropic-managed runtime (no AgentBreeder-managed infra). See the [deploy-target status table](cli-reference#agentbreeder-deploy).
