...
Every framework compiles to every target. **Governance** availability per target is a separate axis — AWS App Runner is single-container (no sidecar, no `guardrails:`/`secrets:`) and Kubernetes + Claude-managed are in flight; see the [deploy-target status table](cli-reference#agentbreeder-deploy).
