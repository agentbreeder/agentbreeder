...
Every AgentBreeder agent container — whether it is written in Python, TypeScript, Go, Kotlin, Rust, or .NET — exposes the **same HTTP surface**. That surface is the runtime contract. It is the only thing the platform (CLI, sidecar, A2A peers, Studio playground) assumes about an agent.
