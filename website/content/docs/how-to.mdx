---
title: How-To Guide
description: Step-by-step guides for common AgentBreeder tasks.
---
...
| Requirement | Min version | Why you need it | Install |
|-------------|-------------|-----------------|---------|
| **Python** | 3.11+ | CLI, API server, Python SDK | `brew install python@3.11` or [python.org](https://www.python.org/downloads/) |
| **Docker** | 24+ | `--target local` deploys, all container builds, full platform stack | [docs.docker.com/get-docker](https://docs.docker.com/get-docker/) |
| **Node.js + npx** | 18+ | MCP servers (most run via `npx -y @modelcontextprotocol/...`), TypeScript SDK | `brew install node` or [nodejs.org](https://nodejs.org/) |
| **Ollama** | any | Local model inference — skip if you have cloud API keys | `brew install ollama` or [ollama.com](https://ollama.com) |
| **gcloud CLI** | any | GCP Cloud Run deploys only | [cloud.google.com/sdk](https://cloud.google.com/sdk/docs/install) |
| **AWS CLI v2** | 2+ | AWS ECS Fargate / App Runner deploys only | [aws.amazon.com/cli](https://aws.amazon.com/cli/) |
| **Azure CLI** | any | Azure Container Apps deploys only | [learn.microsoft.com](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli) |
| **kubectl** | any | Kubernetes deploys only | `brew install kubectl` or [kubernetes.io/docs](https://kubernetes.io/docs/tasks/tools/) |
...
node --version       # need 18+ (for MCP servers and TS SDK)
npx --version        # comes with Node
ollama --version     # optional — for local models
```
...
It asks you 6 questions (or recommends the best stack for your use case) and generates a complete, production-ready project. [See the full walkthrough →](#use-the-agent-architect-agent-build)
...
```bash
# Prerequisites: AWS CLI authenticated, account ID set
...
Your `agent.yaml` should specify AWS:
...
The runtime wraps your client and handles routing, tool injection, and streaming automatically.
...
claude_sdk:
  prompt_caching: true   # Cache system prompts ≥8 192 chars (Sonnet) or ≥16 384 chars (other)
```

Set `AGENT_SYSTEM_PROMPT` at deploy time to provide the system prompt that will be cached.
...
google_adk:
  ...
  memory_service: vertex_ai_bank      # Persist memory in Vertex AI Memory Bank
```

Requires `GOOGLE_CLOUD_PROJECT` env var. Set it in `deploy.env_vars`:
...
Export the crew as `crew`. The runtime automatically propagates `AGENT_MODEL` and `AGENT_TEMPERATURE` (from the top-level `model:` block) into each agent's LLM — you don't need to set them manually.
...
All deployed agents expose a `/stream` endpoint that returns [Server-Sent Events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events). Use it when you want to display partial responses in real time.
...
curl -N -X POST https://<agent-endpoint>/stream \
  ...
  -d '{"input": "Write a detailed report on renewable energy"}'
```

### Stream with JavaScript (browser or Node.js)
...
Other models: `ollama pull llama3`, `ollama pull mistral`, `ollama pull phi4`
...
```bash
# Eject to Python/TypeScript SDK code
agentbreeder eject my-agent --to code

# Or eject just to Low Code YAML (from No Code dashboard export)
agentbreeder eject my-agent --to yaml
```
