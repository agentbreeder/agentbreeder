...
| Studio at `:3001` renders blank | Stale `agentbreeder/agentbreeder-dashboard:latest` cached locally — `docker rmi` it then re-run with `agentbreeder quickstart --dev` to rebuild from source. See [FAQ](faq#studio-at-httplocalhost3001-shows-a-blank-page). |
