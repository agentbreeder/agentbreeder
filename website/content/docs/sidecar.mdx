...
One binary, every language, every wired-up deployer.

<Callout type="info" title="Sidecar injection vs host-platform availability">
  Sidecar injection is cloud-agnostic — it rides along on **Local**, **AWS ECS Fargate**, **GCP Cloud Run**, and **Azure Container Apps**. The exception is **AWS App Runner**, which is single-container: it deploys without a sidecar and rejects `guardrails:`/`secrets:` at validation. For the full per-target matrix see the [deploy-target status table](/docs/cli-reference#agentbreeder-deploy) and [prerequisites per target](/docs/deployment#prerequisites-per-target-as-of-2026-05-18).
</Callout>
