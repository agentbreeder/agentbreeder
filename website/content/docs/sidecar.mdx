...
<Callout type="info" title="Sidecar injection vs host-platform availability">
  Sidecar injection is cloud-agnostic — wherever a deployer is wired up, the sidecar rides along. Which deployers are actually usable today (greenfield vs bring-your-own-cluster) is a separate question; see the [deploy-target status table](/docs/cli-reference#agentbreeder-deploy) and [prerequisites per target](/docs/deployment#prerequisites-per-target-as-of-2026-05-18).
</Callout>
