---
title: Secrets
description: Workspace-scoped secrets backends and auto-mirroring to GCP Cloud Run at deploy time (AWS / Azure auto-mirror on the roadmap).
---
...
today; AWS and Azure auto-mirror are on the roadmap (see
...
At deploy time, on GCP Cloud Run (the target where auto-mirror ships today),
AgentBreeder reads `OPENAI_API_KEY` and `ZENDESK_API_TOKEN` from your
**workspace** backend, mirrors each into GCP Secret Manager under the
deterministic name `agentbreeder/<agent-name>/<secret-name>`, and wires the
running container's env to a Secret Manager reference. The value never lands
on disk in the deploy bundle and never appears in CI logs. For AWS and Azure
targets today, pre-create the secrets in AWS Secrets Manager / Azure Key
Vault and reference them by name in `deploy.secrets:` — see the
...
> **v2.0 cloud coverage:** auto-mirror (creating cloud-provider secret-store
> entries automatically during deploy) is shipped for **GCP Cloud Run** today.
> **AWS** (ECS Fargate, App Runner), **Azure Container Apps**, and **Kubernetes**
> auto-mirror are on the roadmap — for those targets, pre-create your secrets in
> AWS Secrets Manager / Azure Key Vault and reference them by name in
...
When you run `agentbreeder deploy` against GCP Cloud Run and your `agent.yaml`
