...
Prefer the UI? Open **/models** in Studio. The OpenAI-Compatible Catalog table lists every preset with a **Configure** button:
...
| `deprecated` | Either the upstream provider stopped listing the model, **or** an operator manually deprecated it. Still callable, but Studio surfaces a warning. |
...
# Studio — the Sync button on /models triggers the same flow
```
...
- [Gateways](gateways) — when to route through LiteLLM or OpenRouter instead of calling a provider directly
