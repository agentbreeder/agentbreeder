...
description: Use a model gateway as a first-class provider — fan out to many upstreams behind a single API key, with 3-segment model refs, virtual keys, budgets, caching, and guardrails.
---
...
**LiteLLM** — choose when you want to **self-host**, enforce per-agent **virtual keys** + **budgets**, run **caching** locally, or unify cost tracking across your own pool of provider keys. AgentBreeder's docker-compose stack ships a LiteLLM proxy on `:4000`. See [Self-hosted LiteLLM](#self-hosted-litellm) below for the full setup.
