...
*Prereq:* AWS targets require pre-existing IAM roles and an ECR repository. See the [deploy-target status table](cli-reference#agentbreeder-deploy).
...
Full SDK usage: memory, knowledge bases, prompt registry references, MCP servers, and configurable deploy targets — all from Python code. (Greenfield provisioning availability varies by target — see the [deploy-target status table](cli-reference#agentbreeder-deploy).)
