...
> **Status:** Deployer code paths exist for every target listed below. Greenfield provisioning (auto-creating VPCs / clusters / resource-groups) is fully supported only for `local` and `cloud-run`; AWS, Azure, and Kubernetes targets require pre-existing infrastructure. `claude-managed` is an Anthropic-managed runtime. See the [deploy-target status table](cli-reference#agentbreeder-deploy).
...
<Callout type="info" title="Resolver chain">
...
Before (or instead of) deploying, push the agent and its dependent entities to your org's registry so they can be re-used, versioned, and deployed from the dashboard.
...
Once the agent is deployed, you can chat with it through the same registry-aware CLI:
...
agentbreeder registry agent invoke microlearning-ebook-agent \
    ...
    --token "$AGENT_MICROLEARNING_EBOOK_AGENT_TOKEN"
```

The CLI calls `POST /api/v1/agents/{agent_id}/invoke`, which proxies server-side and attaches the bearer token from `--token` or the `AGENT_<UPPER_SNAKE>_TOKEN` env var.
