...
> **Note:** Models that disappear from a provider's `/models` endpoint are auto-deprecated on the next sync. After 30 days of continuous absence they're auto-retired. Set up a daily cron (`agentbreeder model sync`) so this stays current.
