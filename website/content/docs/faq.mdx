...
### Dashboard at `http://localhost:3001` shows a blank page
...
The `--dev` flag tells quickstart to use locally-built images instead of pulling from Docker Hub. Use it whenever you have local source changes you want reflected in the running stack.
...
# Expected: a single line like  version="19.2.5"
```

If both 19.2.4 and 19.2.5 appear, your dashboard image is pre-PR-#350 — rebuild as shown above.
...
docker compose -f deploy/docker-compose.quickstart.yml logs migrate --tail 20
```

Only worry if exit code is non-zero, logs show alembic errors, or the `api` container also fails to start.
