...
memory:
  ...
  backend: redis             # or postgresql — no backend_url → auto-provisioned
```

| `backend` | Provisioned store (per cloud) | Injected env |
|-----------|-------------------------------|--------------|
...
[RAG → Automatic provisioning](/docs/rag) for the per-cloud networking
