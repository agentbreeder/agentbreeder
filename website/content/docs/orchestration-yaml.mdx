---
title: orchestration.yaml Reference
description: Complete reference for orchestration.yaml — the Low Code configuration file for multi-agent pipelines in AgentBreeder.
---
...
All strategies are supported by the Studio visual canvas (No Code) and the Python/TypeScript SDK (Full Code). All tiers share the same deploy pipeline and governance.
...
<Callout type="info">
  The `registry://agents/billing-agent@v2.1` form pins to a specific version. If that version is deprecated, the orchestration validation step will fail with a clear error — protecting you from silent breakage.
</Callout>
...
- All route `target` values reference known agents in the same file
- All `fallback` values reference known agents in the same file
- `supervisor` / `hierarchical` strategies have `supervisor_config.supervisor_agent`
...
The saved file is valid `orchestration.yaml` — open it in the Studio editor, check it into git, or pass it to `agentbreeder orchestration deploy`.

---

*See also: [Full Code Orchestration SDK](/docs/orchestration-sdk) · [CLI reference](/docs/cli-reference#agentbreeder-orchestration) · [agent.yaml reference](/docs/agent-yaml)*
