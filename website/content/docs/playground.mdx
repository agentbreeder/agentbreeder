...
description: The Studio /playground page lets you test a model directly via the LiteLLM gateway, or chat with a deployed agent's runtime through the API proxy.
---

import { Callout } from 'fumadocs-ui/components/callout';

The Studio `/playground` page is the fastest way to test a model or a deployed agent without writing any code. It exposes two modes:
...
Agent mode chats with a **deployed** agent — i.e. an agent in the registry that has a non-empty `endpoint_url`. Studio:
...
Studio now resolves it automatically. See [Secrets — Per-agent auth tokens](./secrets.mdx) for details on the backend lookup.
