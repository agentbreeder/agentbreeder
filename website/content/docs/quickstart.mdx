...
<Callout type="info" title="Recommended: install Python via pyenv">
  ...
  **Linux:**
  ...
  Then `pipx install agentbreeder` (or `python -m pip install agentbreeder`) installs cleanly into the pyenv-managed interpreter. Switching Python versions later is one command (`pyenv global 3.13.x`) and never collides with system Python.
</Callout>
