"""Unit tests for cli/commands/studio.py."""
...
def test_check_docker_not_found():
    ...
    with patch("cli.commands.studio.shutil.which", return_value=None):
...
def test_check_docker_daemon_not_running():
    ...
    with (
        patch("cli.commands.studio.shutil.which", return_value="/usr/bin/docker"),
        patch("cli.commands.studio.subprocess.run", return_value=_DOCKER_FAIL),
    ):
        result = runner.invoke(app, ["studio"])
    assert result.exit_code == 1
...
def test_check_docker_compose_missing():
    ...
    with (
        patch("cli.commands.studio.shutil.which", return_value="/usr/bin/docker"),
        patch(
            "cli.commands.studio.subprocess.run",
            side_effect=[
...
def test_write_compose_file_creates_file(tmp_path: Path):
    """Compose file is written to ~/.agentbreeder/docker-compose.studio.yml."""
    from cli.commands import studio as studio_module

    original_path = studio_module._COMPOSE_PATH
    original_dir = studio_module._AGENTBREEDER_DIR
    try:
        studio_module._AGENTBREEDER_DIR = tmp_path
        studio_module._COMPOSE_PATH = tmp_path / "docker-compose.studio.yml"
        result_path = studio_module._write_compose_file(api_port=8000, dashboard_port=3001)
...
def test_write_compose_file_custom_ports(tmp_path: Path):
    ...
    try:
        studio_module._AGENTBREEDER_DIR = tmp_path
        studio_module._COMPOSE_PATH = tmp_path / "docker-compose.studio.yml"
        studio_module._write_compose_file(api_port=9000, dashboard_port=4000)
        content = (tmp_path / "docker-compose.studio.yml").read_text()
...
def test_poll_health_returns_true_on_200():
    ...
    from cli.commands.studio import _poll_health
    ...
    with patch("cli.commands.studio.urllib.request.urlopen", return_value=mock_resp):
...
def test_poll_health_returns_false_on_timeout():
    ...
    with (
        patch(
            "cli.commands.studio.urllib.request.urlopen",
            side_effect=urllib.error.URLError("refused"),
        ),
        patch("cli.commands.studio.time.sleep"),
        patch("cli.commands.studio.time.monotonic", side_effect=[0, 0, 91]),
    ):
...
def test_ui_happy_path(tmp_path: Path):
    ...
    try:
        ...
        with (
            patch("cli.commands.studio.shutil.which", return_value="/usr/bin/docker"),
            patch("cli.commands.studio.subprocess.run", side_effect=_all_ok_run(None)),
            patch("cli.commands.studio.urllib.request.urlopen", return_value=mock_resp),
        ):
            result = runner.invoke(app, ["studio", "--no-pull"])
    ...
    try:
        ...
        with (
            ...
            result = runner.invoke(
                app, ["studio", "--no-pull", "--port", "4000", "--api-port", "9000"]
            )
