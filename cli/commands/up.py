"""agentbreeder up — start the entire AgentBreeder platform locally.

One command to launch PostgreSQL, Redis, API, Studio, and LiteLLM.
...
GARDEN_HOME = Path.home() / ".agentbreeder"
...
def _generate_env(env_path: Path, interactive: bool = True) -> None:
    ...
    if interactive:
        ...
        console.print(
            Panel(
                ...
                "You can also configure these later in Studio at\n"
    ...
    lines = [
        ...
        "# Generated by `agentbreeder up`. Edit freely or configure via Studio.",
        "",
        ...
        "",
        "# Environment",
        f"AGENTBREEDER_ENV={env_vars['AGENTBREEDER_ENV']}",
        "",
        "# LiteLLM Model Gateway",
        f"LITELLM_BASE_URL={env_vars['LITELLM_BASE_URL']}",
        f"LITELLM_MASTER_KEY={env_vars['LITELLM_MASTER_KEY']}",
        "",
        "# LLM Provider API Keys (configure here or in Studio Settings)",
...
def up(
    ...
    """Start AgentBreeder locally — API, Studio, Database, and more.
    ...
    with console.status("[bold blue]Waiting for services to be healthy..."):
        ...
        dash_ok = _wait_for_health("http://localhost:3001", "Studio")
    ...
    if not dash_ok:
        console.print("  [yellow]⚠[/yellow] Studio did not become healthy within 90s")
        console.print(
            "    [dim]Check logs: docker compose -f deploy/docker-compose.yml logs dashboard[/dim]"
        )
    else:
        console.print("  [green]✓[/green] Studio is healthy")
    ...
    console.print(
        "  [dim]Stop with:[/dim]  agentbreeder down\n"
        "  [dim]Logs:[/dim]      agentbreeder logs   [dim]or[/dim]   "
        "docker compose -f deploy/docker-compose.yml logs -f\n"
        "  [dim]Config:[/dim]    Edit .env and restart with [bold]agentbreeder up[/bold]\n"
    )
